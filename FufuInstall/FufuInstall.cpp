...
bool CheckVCRuntime() {
    ...
    // 查询注册表 HKLM\SOFTWARE\Microsoft\VisualStudio\14.0 或使用 ARP
    HKEY hKey = nullptr;
...
bool InstallVCRuntime() {
    int result = MessageBoxW(g_hMainWnd,
        L"未检测到 Visual C++ 2015-2019 (VC++14) 运行时，是否自动使用 winget 安装？\n\n点击[是]将使用 winget 自动安装\n点击[否]将显示手动安装说明",
