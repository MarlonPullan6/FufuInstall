...
bool InstallWebview2() {
    ...
    std::wstring webview2Path = g_tempPath + L"\\Webview2Setup.exe";
    
    // 检查文件是否存在
    if (GetFileAttributesW(webview2Path.c_str()) == INVALID_FILE_ATTRIBUTES) {
        LogMessage(L"错误: Webview2Setup.exe不存在");
        return false;
    }
    ...
    sei.hwnd = g_hMainWnd;
    sei.lpVerb = L"runas";
    sei.lpFile = webview2Path.c_str();
    sei.lpParameters = L"/silent /install";
    sei.nShow = SW_HIDE;
    
    if (!ShellExecuteExW(&sei)) {
        LogMessage(L"错误: 无法启动WebView2安装程序");
        return false;
    }
    ...
    LogMessage(L"WebView2安装完成，退出代码: " + std::to_wstring(exitCode));
    return true;
}
    ...
    switch (msg) {
        case WM_CREATE: {
            ...
            g_hProgressBar = CreateWindowExW(0, PROGRESS_CLASSW, L"",
                WS_CHILD | PBS_SMOOTH,
                20, 125, 530, 22, hwnd, (HMENU)IDC_PROGRESS_BAR, g_hInstance, nullptr);
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int nCmdShow) {
    ...
    // ========================================================================
    {
        ...
        // 释放WebView2安装程序
        std::wstring webview2 = g_tempPath + L"\\Webview2Setup.exe";
        if (!ExtractResourceToFile(IDR_WEBVIEW2SETUP, L"BINARY", webview2)) {
            MessageBoxW(nullptr, L"无法释放Webview2Setup.exe", L"错误", MB_OK | MB_ICONERROR);
            return 1;
        }
    }
    ...
    DeleteFileW((g_tempPath + L"\\Webview2Setup.exe").c_str());
